...
@output = IO.popen("iostat -Nxk 1")
    while line = @output.gets do
        if  ( line =~ /^(Linux|Time:|avg-cpu|Device| |$)/ )
            ...
            device, rrqm_sec,  wrqm_sec, r_s, w_s, rsec, wsec, avgrq_sz, avgqu_sz, await, svctm, util = line.split
            ...
            puts "PUTVAL " + HOSTNAME.chomp + "/iostatplugin/gauge-" + device + "/util " + "N:" + util
        end
    end
